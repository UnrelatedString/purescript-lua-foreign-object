...
os.execute("sed 's/for str in s:gmatch.+\\\\)/local pattern; if string.len(sep) ~= 0 then pattern = \"([^\" .. sep .. \"]+)\" else pattern = \".\" end; for str in s:gmatch(pattern)/' output/test_.lua > output/test__.lua")
